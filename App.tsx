...
const App: React.FC = () => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      setLoadingStep("OpenAI performing gap analysis...");
      const result = await generateStructuredFeedback(sourceData, feedbackData);
      setReport(result);
    } catch (err: any) {
      console.error("Analysis Failure:", err);
  ...
  const renderDashboard = () => (
    <div className="max-w-5xl mx-auto px-4 mt-12 animate-fade-in pb-20">
      <header className="text-center mb-16">
        ...
        <p className="text-lg text-slate-500 max-w-2xl mx-auto font-medium">
          Professional Anatomy Guru feedback engine. Powered by OpenAI GPT-4o for precise anatomical auditing.
        </p>
      </header>
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa]">
      ...
      <footer className="mt-20 py-10 border-t border-slate-100 no-print text-center opacity-40">
        <p className="text-xs font-bold uppercase tracking-[0.3em]">Anatomy Guru AI (OpenAI GPT-4o)</p>
      </footer>
    </div>
  );
};
