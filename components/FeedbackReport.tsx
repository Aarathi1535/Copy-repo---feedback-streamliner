...
const FeedbackReport: React.FC<FeedbackReportProps> = ({ report }) => {
  ...
  // Base styles for Times New Roman and font sizes
  ...
  return (
    <div className="max-w-[850px] mx-auto my-10 bg-white shadow-none rounded-none border border-slate-300 report-container p-6 sm:p-12 text-[#1e1e1e]" style={reportStyle}>
      ...
      {/* Brand Header - Refined SVG to match the provided brand asset */}
        ...
        <div className="flex items-center justify-center gap-6">
          ...
          <div className="relative w-32 h-32 flex items-center justify-center">
            <svg viewBox="0 0 100 100" className="w-full h-full">
              {/* Skull */}
              <path d="M40 30 C25 30 15 40 15 55 C15 70 25 80 30 85 V93 H50 V85 C55 80 65 70 65 55 C65 40 55 30 40 30 Z" fill="#f8fafc" stroke="#1e1e1e" strokeWidth="2"/>
              <circle cx="30" cy="55" r="6" fill="#1e1e1e" />
              <circle cx="50" cy="55" r="6" fill="#1e1e1e" />
              <path d="M37 68 L40 64 L43 68 Z" fill="#1e1e1e" />
              <path d="M30 75 H50" stroke="#1e1e1e" strokeWidth="1" />
              <path d="M30 78 H50" stroke="#1e1e1e" strokeWidth="1" />
              {/* Graduation Cap */}
              <path d="M10 50 L40 33 L70 50 L40 67 Z" fill="#1e1e1e" />
              <path d="M25 58 V66 C25 66 40 71 55 66 V58" fill="#1e1e1e" />
              <path d="M70 50 V65" stroke="#f59e0b" strokeWidth="2.5" />
              <circle cx="70" cy="65" r="3.5" fill="#f59e0b" />
            </svg>
          </div>
          ...
          <div className="text-left border-l-2 border-slate-900 pl-6">
            ...
            <p className="text-[16px] italic text-slate-500 font-bold mb-2">Nothing Left Undissected !!</p>
            {/* Scalpel Illustration */}
      ...
      <div className="mt-12 space-y-10 no-print">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="p-6 bg-emerald-50/30 rounded-xl border border-emerald-100">
            ...
            <ul className="list-disc list-inside space-y-3" style={{ fontSize: '14px' }}>
        ...
        {report.generalFeedback.unattemptedAdvice.length > 0 && (
          <section className="bg-red-50 p-6 border border-red-100 rounded-xl">
            <h4 className="text-red-700 font-black uppercase tracking-widest mb-4" style={{ fontSize: '16px' }}>Strategic Advice for Skipped Items</h4>
            <div className="space-y-4">
              {report.generalFeedback.unattemptedAdvice.map((item, i) => (
                <div key={i} className="flex flex-col gap-1 border-b border-red-100 pb-3 last:border-0">
                  <span className="font-black text-red-600 uppercase" style={{ fontSize: '14px' }}>Q.{item.qNo} Gap Recovery</span>
